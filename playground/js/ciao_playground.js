...
async function loadCustomUI(pg) {
  ...
  saveBtn.addEventListener('click', function(e) {
    ...
    if(name != null) {
      let file = new Blob([generateCode()], {
        type: 'text/plain'
      });
      a_download.href = URL.createObjectURL(file);
      a_download.download = name + '.dn';
    } else {
      e.preventDefault();
      a_download.href = '';
      a_download.download = '';
...
function setEventsPasos(container) {
  ...
  pasoA.addEventListener('focus', () => {
    buildPasoA(pasoA);
  })
  pasoA.addEventListener('change', () => {
    if(pasoA.value == "E" || pasoA.value == "I") {
      buildPasoB(pasoB, pasoA);
      pasoB.dispatchEvent(new Event('change'));
    } else {
      pasoB.style.display = "none";
      buildPasosInputs(pasosInputs, pasoA.value);
    }
  })
  pasoB.addEventListener('change', () => {
...
function buildPasoA(pasoA) {
  ...
  pasoA.innerHTML = initialPasos;
  options.map(option => {
    pasoA.innerHTML += '<option value="' + option + '">' + option + '</option>'
  });
}
...
function buildPasoB(pasoB, pasoA) {
  if(pasoA.value == 'E') {
    ...
    pasoB.style.display = "";
  } else if (pasoA.value == 'I') {
    pasoB.innerHTML = inclusion;
    pasoB.style.display = "";
  } else {
    pasoB.innerHTML = "";
    pasoB.style.display = "none";
...
async function addDeduccion(btn) {
  let deduccion = await createDeduccionUI();
  let dedCont = document.getElementsByClassName('deduccionContainer')[0];
...
async function addRegla(btn) {
  let regla = await createReglaUI();
  let reglaCont = document.getElementsByClassName('reglaContainer')[0];
...
function addPaso(button) {
  let root = button.parentNode.parentNode;
  let paso = root.children[0];
  let newPaso = paso.cloneNode(true);
  $(newPaso).find('.pasoA')[0].value = "";
  $(newPaso).find('.pasoB')[0].style.display = "none";
  let inputs = $(newPaso).find('.pasosInputs').find('input');
...
async function buildDeduccion(name, premisas, consecuente, pasos) {
  let deduccion = await createDeduccionUI();
  let dedCont = document.getElementsByClassName('deduccionContainer')[0];
  dedCont.insertBefore(deduccion, $('#addDedBtn')[0].parentNode);

  $(deduccion).find('.deduccionName').val(name.trim());

  for (let i = 1; i < premisas.length; i++) {
    $(deduccion).find('.addPremisa').click();
  }
  let premisasInputs = $(deduccion).find('.premisa');
  for (let i = 0; i < premisas.length; i++) {
    premisasInputs[i].value = premisas[i].trim();
  }

  $(deduccion).find('.consecuente').val(consecuente.trim());

  for (let i = 1; i < pasos.length; i++) {
    $(deduccion).find('.addPaso').click();
  }
  let pasosCont = $(deduccion).find('.paso');
  fillPasos(pasos, pasosCont);
}

async function buildRegla(name, premisas, consecuente, pasos) {
  let regla = await createReglaUI();
  let reglaCont = document.getElementsByClassName('reglaContainer')[0];
  reglaCont.insertBefore(regla, $('#addRegBtn')[0].parentNode);

  $(regla).find('.reglaName').val(name.trim());

  for (let i = 1; i < premisas.length; i++) {
    $(regla).find('.addPremisa').click();
  }
  let premisasInputs = $(regla).find('.premisa');
  for (let i = 0; i < premisas.length; i++) {
    premisasInputs[i].value = premisas[i].trim();
  }

  $(regla).find('.consecuente').val(consecuente.trim());

  for (let i = 1; i < pasos.length; i++) {
    $(regla).find('.addPaso').click();
  }
  let pasosCont = $(regla).find('.paso');
  fillPasos(pasos, pasosCont);
}

async function fillPasos(pasos, pasosCont) {
  for (let i = 0; i < pasos.length; i++) {
    let pasoA = $(pasosCont[i]).find('.pasoA')[0];
    let pasoB = $(pasosCont[i]).find('.pasoB')[0];
    let pasosInputs = $(pasosCont[i]).find('.pasosInputs')[0];
    let split = pasos[i].split('(');
    if(split[0].trim() == "'Premisa'") {
      pasoA.value = 'Premisa';
      pasoA.dispatchEvent(new Event('change'));
      let inputs = $(pasosInputs).find('input');
      inputs[0].value = split[1].slice(0,-1);

    } else if(split[0].trim() == "'Supuesto'") {
      pasoA.value = 'Supuesto';
      pasoA.dispatchEvent(new Event('change'));
      let inputs = $(pasosInputs).find('input');
      inputs[0].value = split[1].slice(0,-1);

    } else if(split[0].trim().startsWith("'E'")) {
      pasoA.value = 'E';
      pasoA.dispatchEvent(new Event('change'));
      let b = split[0].split("'")[2];
      pasoB.value = b;
      pasoB.dispatchEvent(new Event('change'));
      let params = split[1].slice(0,-1).split(',');
      let inputs = $(pasosInputs).find('input');
      for (let j = 0; j < inputs.length; j++) {
        inputs[j].value = params[j];
      }
      
    } else if(split[0].trim().startsWith("'I'")) {
      pasoA.value = 'I';
      pasoA.dispatchEvent(new Event('change'));
      let b = split[0].split("'")[2];
      pasoB.value = b;
      ...
      pasoA.dispatchEvent(new Event('focus'));
      pasoA.value = split[0].trim().slice(1,-1);
      pasoA.dispatchEvent(new Event('change'));
      let params = split[1].slice(0,-1).split(',');
      let inputs = $(pasosInputs).find('input');
      for (let j = 0; j < inputs.length; j++) {
...
//IO
async function loadFromFile(code) {
  ...
  if(reglas != null) {
    for (let i = 0; i < reglas.length; i++) {
      ...
      let pasosRegex = new RegExp('[^,()]+\\([^()]*\\)', 'g');
      let pasos = pasosRaw.match(pasosRegex);
      let consRegex = new RegExp('\\][^\\[\\]]*\\[', 'g');
      let consecuente = el.match(consRegex)[0].split(',')[1].trim();

      await buildRegla(name, premisas, consecuente, pasos);
    }
  } else {
    $('#addRegBtn').click();
  }
  ...
  if(deducciones != null) {
    for (let i = 0; i < deducciones.length; i++) {
      ...
      let pasos = pasosRaw.match(pasosRegex);
      let consRegex = new RegExp('\\][^\\[\\]]*\\[', 'g');
      let consecuente = rest.match(consRegex)[0].split(',')[1].trim();

      await buildDeduccion(name, premisas, consecuente, pasos);
    }
  } else {
    $('#addDedBtn').click();
  }
...
function createReglaScript(regla) {
  let nombre = $(regla).find('.reglaName')[0].value.trim();
  let premisas = $(regla).find('.premisa');
  ...
  if(nombre == '' || consecuente == '' ||
    codePremisas.toString() == '' ||
    codePasos.toString() == '') {
      alert('La regla con nombre: "' + nombre + '" tiene campos vacios');
      return '';
    }
...
function generateCode() {
  ...
  let reglas = $('.regla');
  for (let i = 0; i < reglas.length; i++) {
    newCode += createReglaScript(reglas[i]);    
  }
