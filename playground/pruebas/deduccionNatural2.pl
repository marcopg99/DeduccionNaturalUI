...
%% Ejemplos
ejemplo1 :-
    main([s ∧ p ∨ q, p → ¬ r, q → ¬ r], s ∧ ¬ r, ['Premisa'(1), 'E' ∧ b(1), 'Premisa'(2), 'Premisa'(3), 'E' ∨ (2,3,4), 'E' ∧ a(1), 'I' ∧ (6,5)]).

ejemplo2 :-
    main([¬p → q ∧ ¬q],p,['Premisa'(1),'I' ¬ (1), 'E' ¬ (2)]).
...
ejemploMT :-
    main([r → (q ∧ s), ¬(q ∧ s)], ¬r, ['Premisa'(1), 'Premisa'(2), 'MT'(1,2)]).

ejemploSupuesto :- %% Falla porque no esta cerrado el supuesto :-)
    main([s ∧ p ∨ q, p → ¬ r, q → ¬ r], s ∧ ¬ r, ['Premisa'(1), 'E' ∧ b(1), 'Premisa'(2), 'Premisa'(3), 'Supuesto'(p1),'Supuesto'(p2),'Supuesto'(p3),'Supuesto'(p4),'Supuesto'(s ∧ ¬r)]).
...
'I' ∨ a(A,Formula) :-
    formula(A,FA),
    add_formula(FA ∨ Formula).
'I' ∨ b(Formula,B) :-
    formula(B,FB),
    add_formula(Formula ∨ FB).
% Negación
'I' ¬ (A) :-
    formula(A, FA → B ∧ ¬ B),
    add_formula(¬ FA).
